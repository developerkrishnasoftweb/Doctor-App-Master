...
class SearchBarDiagnosis extends StatefulWidget {
  ...
  SearchBarDiagnosis({Key key, this.pId, this.docId,this.clinicDocId }) : super(key: key);

  @override
  _SearchBarDiagnosisState createState() => _SearchBarDiagnosisState();
}

class _SearchBarDiagnosisState extends State<SearchBarDiagnosis> {
  String query = '';
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      child: AlertDialog(
        ...
        //   children: <Widget>[
        //     Container(child: Text('Diagnosis')),
        //     InkWell(
          ...
          itemBuilder: (_, index) {
            ...
            return GestureDetector(
              onTap: () async {
                ...
                // pv.updateDiagnosis(p.last, bh, recom, med);
                // Navigator.pop(context);
                showDialog(
                  context: context,
                  builder: (BuildContext context) {
...
StreamBuilder<List<PatientsVisitData>> _buildTaskListGrid4(BuildContext context,	
  ...
  return StreamBuilder(	
    ...
    builder: (context, AsyncSnapshot<List<PatientsVisitData>> snapshot) {	
      switch (snapshot.connectionState) {	
        ...
        default:	
          return Container(	
             height: (snapshot.data.last.diagnosis == null)?0:(snapshot.data.last.diagnosis.data.length==0)?0:(snapshot.data.last.diagnosis.data.length<=3)?50:120,	
                                     ...
                                     child: Row(	
                                       ...
                                       children: [	
                                         RichText(	
                                           ...
                                           text: TextSpan(	
                                             text: snapshot.data.last.diagnosis	
                                                 .data[index].title,	
                                             style: TextStyle(	
                                               color: white,	
                                               fontSize: 14,	
          ...
          SizedBox(
            ...
            child: RaisedButton(
                ...
                onPressed: () {
                  ...
                  database.addBrief(diseaseName, period, widget.docId,widget.clinicDocId );
                  Navigator.pop(context);
                }),
          ),
        ],
